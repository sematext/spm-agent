...
function SpmAgent (spmUrl) {
  ...
  self.spmSender.on('sendFailed', function () {
    counters.error += 1
  ...
  if (!config.tokens.spm) {
    var msg = 'Please set SPM App Token e.g. export spmagent_tokens_spm="YOUR-SPM-APP-TOKEN"'
    console.error(msg)
    logger.error(msg)
  }
}
...
SpmAgent.prototype.createAgent = function (agent) {
  ...
  this.metricCollector.addAgent(a)
  return a
}
