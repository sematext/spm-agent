...
function SpmAgent (spmUrl) {
  ...
  var InfluxDbSender = require('./sender/influxsender.js')
  this.spmSender = new SpmSender(config.get('tokens.spm'), 0, spmUrl)
  this.influxSender = new InfluxDbSender(config.get('tokens.spm'), config.get('transmitInterval'), spmUrl)
  ...
  function shipMetrics (metric) {
    try {
      ...
      if (metric.tags && metric.measurement) {
        logger.debug('Influx metric detected ' + JSON.stringify(metric))
