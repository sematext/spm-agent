...
function SpmAgent (spmUrl) {
  ...
  const SpmSender = require('./sender/spmsender.js')
  const InfluxSender = require('./sender/influxsender.js')
  this.spmSender = new SpmSender(config.get('tokens.spm'), 0, spmUrl)
  this.influxSender = new InfluxSender(
    config.get('tokens.monitoring') || config.get('tokens.spm'),
    config.get('transmitInterval'),
    spmUrl
  )
  ...
  this.metricCollector = new Harvester(
    shipMetrics.bind(this),
    config.agentsToLoad
  )
  ...
  if (!config.tokens.spm && !process.env.LOGSENE_STATS_TOKEN) {
    const msg =
      'Please set Monitoring App Token e.g. export SPM_TOKEN="YOUR-SPM-APP-TOKEN"'
    console.error(msg)
    logger.error(msg)
  }
}
