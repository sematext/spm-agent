...
function InfluxSender (spmToken, processMetricsInterval, metricsApiEndpoint) {
  ...
  this.connectionOptions = {
    ...
    port: Number(config.influx.port) || Number(apiUrl.port),
    database: config.influx.dbName || 'metrics',
    protocol: config.influx.protocol || apiUrl.protocol
  }
  this.influxUrl = this.connectionOptions.protocol + '://' + this.connectionOptions.host + ':' + this.connectionOptions.port
  logger.debug('InfluxSender: connection options ' + JSON.stringify(this.connectionOptions))
...
InfluxSender.prototype.collectMetric = function (metric) {
  if (metric && metric.measurement && metric.tags && metric.fields) {
    ...
    logger.debug('InfluxSender: add metric ' + JSON.stringify(this.connectionOptions))
  } else {
...
InfluxSender.prototype.sendMetrics = function () {
  ...
  this.influx.writePoints(this.datapoints)
    .then(() => {
  ...
  this.sendSematextEvent('server-info',
    ...
    function (err, result) {
      if (err) {
        ...
        logger.debug('InfluxSender client info event:' + JSON.stringify(serverInfo) + ' ' + result.body)
      }
      if (action === 'Stop') {
