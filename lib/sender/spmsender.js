...
SpmSender.prototype.sendClientInfo = function () {
  ...
  try {
    ...
    var programVersion = require(path.join(path.dirname(require.main.filename), '/../package.json')).version
    var name = require(path.join(path.dirname(require.main.filename), '/../package.json')).name
    console.log (name + ' ' + programVersion)
  } catch (e) {
    //console.log(err)
    name = 'spm-agent'
