...
function SpmSender (spmToken, processMetricsInterval, metricsApiEndpoint) {
  ...
  this.tid = null
  this.metricsUrl = (metricsApiEndpoint || process.env.SPM_SPM_RECEIVER_URL || config.get('spmSenderBulkInsertUrl') || defaultSpmSenderUrl) + '?agentType=nodejs&v=' + (programVersion || clientVersion) + '&host=' + hostname + '&token=' + spmToken
  // duplicate metrics to a second App
  if (process.env.SPM_MIRROR_TOKEN) {
    this.metricsUrlMirror = (metricsApiEndpoint || config.get('spmSenderBulkInsertUrl') || defaultSpmSenderUrl) + '?agentType=nodejs&v=' + (programVersion || clientVersion) + '&host=' + hostname + '&token=' + process.env.SPM_MIRROR_TOKEN
  }
