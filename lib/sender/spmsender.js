  ...
  if (fs.existsSync(getFileNameForDb()) && fs.statSync(getFileNameForDb()).size >= 1024 * 1024 * 50) {
    db.remove({ts: {$lt: new Date().getTime() - (1000 * 60 * 60)}}, function (err, numRemoved) {
      try {
        if (err)
        {
          logger.error('Removed old records from NeDB because NeDB file grows over maximum failed: %s', (err.msg||err.toString()))
        } else {
          logger.info ('Removed old records from NeDB because NeDB file grows over maximum: %d records removed', numRemoved||0)
        }
      } catch (error) {
        logger.error ('Unknown error in function checkMaxDbSize ', {error: error, dbError: err||'-', numRemoved: numRemoved||-1})
      }
    })
  }
}
      ...
      request.post(options[xi], function (err, res, body) {
        if ((!err) && (res && res.statusCode < 300)) {
          ...
          dataToRT.forEach(function (m, i) {
            db.remove({_id: m._id}, {multi: true}, function (err, numRemoved) {
              if (err)
                return
              removedCounter++
