...
function checkSpmDockerHostname(cb) {
  fs.stat('/opt/spm/.docker', function (err, stats) {
    if (!err) {
      var data = fs.readFile('/opt/spm/.docker', function (error, data) {
        if (!error && data) {
          var lines = data.toString().split('\n')
          if (lines.length > 1) {
            var keyVal = lines[1].split('=')
            if (keyVal.length > 1 && keyVal[0] ==='docker_hostname') {
                var spmReportedHostname = keyVal[1]
                return cb(null, spmReportedHostname)
            } else {
...
function SpmSender (spmToken, processMetricsInterval, metricsApiEndpoint) {
  ...
  checkSpmDockerHostname(function (err, dockerHostName) {
    if (!err) {
      ...
      process.env.SPM_REPORTED_HOSTNAME = dockerHostName
    } else {
      logger.log('debug', 'failed to get docker hostname: ' + err)
    }
    logger.log('info', 'SPM_REPORTED_HOSTNAME: ' + hostname)
    self.metricsUrl = (metricsApiEndpoint || process.env.SPM_SPM_RECEIVER_URL || config.get('spmSenderBulkInsertUrl') || defaultSpmSenderUrl) + '?agentType=nodejs&v=' + clientVersion + '&host=' + hostname + '&token=' + spmToken
    // duplicate metrics to a second App
    if (process.env.SPM_MIRROR_TOKEN) {
      self.metricsUrlMirror = (metricsApiEndpoint || config.get('spmSenderBulkInsertUrl') || defaultSpmSenderUrl) + '?agentType=nodejs&v=' + clientVersion + '&host=' + hostname + '&token=' + process.env.SPM_MIRROR_TOKEN
    }
  })
  this.datapoints = []
