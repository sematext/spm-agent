...
function SpmSender (spmToken, processMetricsInterval, metricsApiEndpoint) {
  ...
  checkSpmDockerHostname(function (err, dockerHostName) {
    ...
    self.metricsUrl =  metricsApiEndpointVerified + '?agentType=nodejs&v=' + clientVersion + '&host=' + hostname + '&token=' + spmToken
    // duplicate metrics to a second App
    if (process.env.SPM_MIRROR_TOKEN) {
      self.metricsUrlMirror = metricsApiEndpointVerified + '?agentType=nodejs&v=' + clientVersion + '&host=' + hostname + '&token=' + process.env.SPM_MIRROR_TOKEN
    }
  })
  this.datapoints = []
