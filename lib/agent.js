  ...
  var agentSuperClass = {
    ...
    addInfluxTags: function (metric) {
      ...
      if (
        ...
        logger.log(process.env.isDocker)
        if (process.env.isDocker === true) {
          // don't set default tags, when already provided
          if (!metric.tags['container.id']) {
            metric.tags['container.id'] = process.env.containerId
          }
          if (!metric.tags['container.name']) {
            metric.tags['container.name'] = process.env.containerName
          }
          if (!metric.tags['container.image']) {
            metric.tags['container.image'] = process.env.containerImage
          }
        }
      }

      logger.log(metric.tags)
