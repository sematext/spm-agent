  ...
  var agentSuperClass = {
    ...
    addInfluxTags: function (metric) {
      ...
      if (
        ...
        console.log('process.env.kubernetesTags: ', process.env.kubernetesTags)
        ...
        if (isK8s) {
          ...
          if (!metric.tags['container.id'] && kubernetesTags?.container?.id) {
            metric.tags['container.id'] = kubernetesTags.container.id
          }
          if (!metric.tags['container.name'] && kubernetesTags?.container?.name) {
            metric.tags['container.name'] = kubernetesTags.container.name
          }
          if (!metric.tags['container.image.name'] && kubernetesTags?.image?.name) {
            metric.tags['container.image.name'] = kubernetesTags.image.name
          }
          if (!metric.tags['container.image.tag'] && kubernetesTags?.image?.tag) {
            metric.tags['container.image.tag'] = kubernetesTags.image.tag
          }
          if (!metric.tags['container.image.digest'] && kubernetesTags?.image?.digest) {
            metric.tags['container.image.digest'] = kubernetesTags.image.digest
          }
          if (!metric.tags['container.pod.name'] && process.env.POD_NAME) {
            metric.tags['container.pod.name'] = process.env.POD_NAME
          }
          if (!metric.tags['container.pod.namespace'] && process.env.POD_NAMESPACE) {
            metric.tags['container.pod.namespace'] = process.env.POD_NAMESPACE
          }
        }
