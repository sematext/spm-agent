...
function Agent (plugin) {
  ...
  var agentSuperClass = {
    ...
    addInfluxTags: function (metric) {
      ...
      if (metric.measurement.startsWith('nodejs') ||
        ...
        if (!metric.tags['pid']) {
          metric.tags['pid'] = process.pid
        }
        if (!metric.tags['ppid']) {
          metric.tags['ppid'] = process.ppid
        }
      }
    ...
    start: function () {
      ...
      let tagNames = (process.env.MONITORING_TAGS_FROM_ENV || config.influx.tagsFromEnv)
