...
const RC = require('rc-yaml-2')
const fs = require('fs')
const os = require('os')

function setReceiverUrlsForRegion (region) {
  ...
  if (region === 'US' || region === 'us') {
    process.env.SPM_RECEIVER_URL = 'https://spm-receiver.sematext.com/receiver/v1'
    process.env.EVENTS_RECEIVER_URL = 'https://event-receiver.sematext.com'
...
if (process.env.REGION !== undefined) {
  setReceiverUrlsForRegion(process.env.REGION)
}

const spmDefaultConfig = {
  tokens: {
...
const SpmConfig = function (appType) {
  const rc = new RC(appType, spmDefaultConfig)
